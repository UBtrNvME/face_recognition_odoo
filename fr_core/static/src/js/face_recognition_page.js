odoo.define('fr_core.face_recognise_sign_up', function (require) {
    ...
    publicWidget.registry.SignUpFaceRecognition = publicWidget.Widget.extend({
        ...
        process_response: function (result) {
            ...
            this.$('.message-box').toggleClass('alert-success', result.status.success)
            this.$('.message-box').toggleClass('alert-warning', result.status.success && result.status.code >= 300)
            this.$('.message-box').toggleClass('alert-danger', !result.status.success)
